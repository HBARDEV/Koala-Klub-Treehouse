...
def flag(request, comment_id, next=None):
    ...
    """
    comment = get_object_or_404(tagging.get_model(),
                                pk=comment_id,
...
@csrf_protect
@login_required
@permission_required("tagging.can_moderate", raise_exception=True)
...
def perform_flag(request, comment):
    ...
    """
    flag, created = tagging.models.CommentFlag.objects.get_or_create(
        comment=comment,
        user=request.user,
        flag=tagging.models.CommentFlag.SUGGEST_REMOVAL
    )
...
def perform_delete(request, comment):
    flag, created = tagging.models.CommentFlag.objects.get_or_create(
        comment=comment,
        user=request.user,
        flag=tagging.models.CommentFlag.MODERATOR_DELETION
    )
...
def perform_approve(request, comment):
    flag, created = tagging.models.CommentFlag.objects.get_or_create(
        comment=comment,
        user=request.user,
        flag=tagging.models.CommentFlag.MODERATOR_APPROVAL,
    )
